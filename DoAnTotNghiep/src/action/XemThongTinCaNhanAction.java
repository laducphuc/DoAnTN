		...
		if (dangNhapForm != null) {
			...
			if (thanhVienForm.getSubmit() != null) {
				if ("Đổi mật khẩu".equals(StringProcess.toUTF8(thanhVienForm.getSubmit()))) {
					...
					String maTaiKhoan = dangNhapForm.getTaiKhoan().getMaTaiKhoan();
					...
					if (Validations.validateNull(nhapMatKhauCu)) {
						actionErrors.add("maKhauCuError", new ActionMessage("error.maKhauCu.Trong"));
						System.out.println("MKCu trong !");
					} else if (!Validations.validateTrungMatKhau(matKhauCu, nhapMatKhauCu)) {
						actionErrors.add("maKhauCuError", new ActionMessage("error.maKhauCuError.khongTrung"));
						System.out.println("MKCu KTrung !");
					}
					if (Validations.validateNull(nhapMatKhauMoi)) {
						actionErrors.add("maKhauError", new ActionMessage("error.matKhau.Trong"));
					} else if (Validations.validateSpace(nhapMatKhauMoi)) {
						actionErrors.add("matKhauError", new ActionMessage("error.toanSpace"));
					}
					if (!Validations.validateTrungMatKhau(nhapMatKhauMoi, nhapLaiMatKhau)) {
						actionErrors.add("nhapLaiMatKhauMoiError", new ActionMessage("error.trungMatKhau.loi"));
					}
					saveErrors(request, actionErrors);

					if (actionErrors.size() > 0) {
						if ("2".equals(thanhVien.getChucVu())) {
							thanhVienForm.setThanhVien(thanhVien);
							return mapping.findForward("hienThiThongTinCaNhanCongTacVien");
						} else {
							thanhVienForm.setThanhVien(thanhVien);
							return mapping.findForward("hienThiThongTinCaNhanThanhVien");
						}
					}
